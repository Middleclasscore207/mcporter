...
- Run `mcporter list --json | rg '"source":'` to confirm the resolved file for each imported server.
- Use `mcporter list --source import` to limit the output to merged definitions when you’re debugging precedence.
